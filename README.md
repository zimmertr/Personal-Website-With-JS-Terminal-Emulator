...
1) Deploy Apache webserver; install perl, perl-cgi, snmpwalk, nslookup
2) Copy cgi scripts to `/var/www/cgi-bin`; copy `index.html` and `files` directory to `/var/www/html`
3) Enable execution of CGI scripts.
4) Install JackWink's WeatherPy: https://github.com/JackWink/Weather
5) Create wunderground api token, add cron to update weather txt file periodically.
